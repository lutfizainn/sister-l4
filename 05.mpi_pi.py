...
def local_loop(num_steps,begin,end):
    ...
    jum = 0
    # 4/(1+x^2)
    for i in range(begin,end): # begin <= x < end
        a = (i+0.5)*step
        jum += (4.0/(1.0+a*a))
        
    return jum, begin, end 
...
def Pi(num_steps):
    ...
    local_sum,begin,end = local_loop(num_steps,int(rank*var),int((rank+1)*var))
    
    # lakukan penjumlahan dari local_sum proses-proses yang ada ke proses 0
    # bisa digunakan reduce atau p2p sum
    jum = comm.allreduce(local_sum,op=MPI.SUM)
    
    # jika saya proses dengan rank 0  maka tampilkan hasilnya
    if rank == 0:
        pi = jum / num_steps
        print ("pi :",pi)
        print("rank: ",rank," -> beginning iteration:",begin," end of iteration",end-1)
    else:   
        print("rank: ",rank," -> beginning iteration:",begin," end of iteration",end-1)
